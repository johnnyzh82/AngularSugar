(function() {
	'use strict';
	angular.module('appModule').directive('percentCircle', ['$timeout', function ($timeout) {
		return {
			restrict: 'E',
			replace: true,
			...
			link: function($scope, element, attrs) {
				...
				function buildColorObj() {
					...
					colors.center    = (!$scope.colors.center)    ? '#ffffff' : $scope.colors.center;
					colors.highlight = (!$scope.colors.highlight) ? '#2BCBED' : $scope.colors.highlight;
					colors.remaining = (!$scope.colors.remaining) ? '#e0e0e0' : $scope.colors.remaining;
				}
